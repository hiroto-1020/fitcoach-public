...
import {
 ...
 Platform, ActivityIndicator, Alert, SafeAreaView, Modal, Pressable, Image, Animated,
 RefreshControl
} from 'react-native';
...
export default function ChatInboxScreen() {
  ...
  if (!GOTORE_ENABLED) return <ComingSoon />;
  ...
  if (notAuthed) {
    return (
      <LinearGradient colors={['#0b1220', '#111827']} style={{ flex:1, alignItems:'center', justifyContent:'center', padding:16 }}>
        <Text style={{ fontSize:16, fontWeight:'700', color:'#fff' }}>チャットを使うにはログインが必要です</Text>
      ...
      <FlatList
        ...
        ListEmptyComponent={
          <View style={{ flex:1, alignItems:'center', justifyContent:'center', padding:24 }}>
            <Text style={{ fontSize:18, fontWeight:'800', marginBottom:6, color:'#fff' }}>トークはまだありません</Text>
            <Text style={{ color:'#94a3b8', textAlign:'center' }}>
              まずは「探す」→「いいね」で相手と会話を始めましょう。
            </Text>
          </View>
        }
        renderItem={({ item }) => (
...
function ChatRow({ item, onPress }: { item: MatchRow; onPress: () => void }) {
        ...
        >
          <View style={{ flexDirection:'row', alignItems:'center' }}>
            ...
            <View style={{ flex: 1, paddingRight: 8 }}>
              <View style={{ flexDirection:'row', alignItems:'center' }}>
                <Text numberOfLines={1} style={{ color:'#fff', fontSize:16, fontWeight:'900', maxWidth:'78%' }}>
              ...
              <Text numberOfLines={1} style={{ marginTop: 6, color: unread ? '#ffffff' : '#e2e8f0', opacity: unread ? 1 : 0.85, fontWeight: unread ? '800' : '600' }}>
